...
class PatientOphthalmicDiagnosesService extends DeclarativeModelService
{
		...
		if (!empty($resource->diagnoses)) {
			foreach ($resource->diagnoses as $diagnosis) {
				if ($disorder = \Disorder::model()->find('term=?',array($diagnosis->disorder))) {
					if (!$disorder->specialty_id || $disorder->specialty->code != 130) {
						throw new \Exception('PatientOphthalmicDiagnoses passed a resource containing non-ophthalmic diagnoses');
					}
				}
			}
		}
