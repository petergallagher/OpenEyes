...
class DeclarativeTypeParser_DataObjectExclusiveExclusiveTest extends \CDbTestCase
{
	public function testModelToResourceParse_ObjectDataType()
	{
		...
		$mc->expects($this->once())
			...
			->with($data, new Address)
			->will($this->returnValue('cheddar'));

		$p = new DeclarativeTypeParser_DataObjectExclusive($mc);

		$this->assertEquals('cheddar',$p->modelToResourceParse($object,'data','Address'));
	}
	...
	public function testModelToResourceParse_Null()
	{
		...
		$this->assertNull($p->modelToResourceParse($object,'pencil','Address'));
	}
	...
	public function testModelToResourceParse_NewInstanceFromArray()
	{
		...
		$this->assertEquals(new Address($data), $p->modelToResourceParse($object,'data','Address'));
	}
	...
	public function testResourceToModel_RelatedObjects_CopyAttribute()
	{
		...
		$p = new DeclarativeTypeParser_DataObjectExclusive($model);
	...
	public function testResourceToModel_RelatedObjects_BothTrue_Save()
	{
		...
		$mc = $this->getMockBuilder('services\ModelConverter')
			->disableOriginalConstructor()
			->setMethods(array('expandObjectAttribute','modelToResource','saveModel', 'deleteModel'))
			->getMock();
	...
	public function testJsonToResourceParse_HasData()
	{
		...
		$this->assertEquals('three', $p->jsonToResourceParse($obj, 'one', 'testJsonToResourceParse_Exclusive_HasData_DataClass', null));
	}
...
class testJsonToResourceParse_Exclusive_HasData_DataClass
{
