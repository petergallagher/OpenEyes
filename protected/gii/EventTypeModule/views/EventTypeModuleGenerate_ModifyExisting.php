		...
		<select class="EventTypeModuleEventType" name="EventTypeModuleEventType">
			...
			<?php foreach (EventType::model()->findAll(array('order'=>'name asc')) as $event_type) {?>
				<option value="<?php echo $event_type->id?>"<?php if (@$_POST['EventTypeModuleEventType'] == $event_type->id) {?> selected="selected"<?php }?>><?php echo $event_type->name?></option>
			<?php }?>
		</select>
		<div id="EventTypeModuleEventTypeData">
			...
			<div id="EventTypeModuleEventTypeElementTypes"<?php if (!@$_POST['EventTypeModuleEventType']) {?> style="display: none;"<?php }?>>
